...
/*		for (int i = 0; i < K; i++){
		...
		*/
		...
		int topNum = 10; //Find the top 20 topic words in each topic;改为5个关键词
		for(int i = 0; i < K; i++){
			...
			TemporarySpace.setConsoleString("topic " + i + "\t:\t");
			TemporarySpace.setLDASubjectString("topic " + i + "\t:\t");
			for(int t = 0; t < topNum; t++){
				writer.write(docSet.indexToTermMap.get(tWordsIndexArray.get(t)) + " " + phi[i][tWordsIndexArray.get(t)] + "\t");
				TemporarySpace.setConsoleString(docSet.indexToTermMap.get(tWordsIndexArray.get(t)) + " " + phi[i][tWordsIndexArray.get(t)] + "\t");
				TemporarySpace.setLDASubjectString(docSet.indexToTermMap.get(tWordsIndexArray.get(t)) + " " + phi[i][tWordsIndexArray.get(t)] + "\t");
			}
			writer.write("\n");
			TemporarySpace.setConsoleString("\n");
			TemporarySpace.setLDASubjectString("\n");
		}
		writer.close();
	}
